...
file_to_write= open("/home/nvidia/tensorflowCode/roc.txt","w")
# Reading the dataset
...
# Parameters
learning_rate = 0.001
total_img = 8000 # Total Number of images
...
def conv_net(x, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope('ConvNet', reuse=reuse):
        ...
        conv1 = tf.layers.conv2d(x, 128, 3, activation=tf.nn.relu)
...
with tf.Session(config=config) as sess:
    ...
    for step in range(1, num_steps_test+1):
        ...
        #writer.add_summary(mrg, global_step=step)
    ...
    width = 0.01
    plt.errorbar(bin_centres_sig,counts_sig,yerr=menStd_sig,fmt='o',label='Isolated electrons from Z decay(Training Set)')
    ...
    plt.errorbar(bin_centres_bg,counts_bg,yerr=menStd_bg,fmt='o',label = 'Non-Isolated electrons from QCD(Training Set)')
    plt.hist(sig_pred_test,label='Isolated electrons from Z decay',bins=bins,histtype='step',color='blue')
    plt.hist(bg_pred_test,label='Non-Isolated electrons from QCD decay',bins=bins,histtype='step',color='red')
